...
Also check `data/distillations/<story-slug>.md` — a chapter-by-chapter set of tagged quotes and observations with no verdict attached, written by `work-distiller` (full passes) and/or by you and `benchmark-filter` (partial, sample-only passes — see the key rule below). Rank your sources: a preliminary review outranks a distillation (it has an actual worked judgment), a fully-covered distillation outranks your own sampling (it covers the whole work, not just a slice), and your own representative-sample read is the fallback for whatever isn't covered by either. Per chapter, only skip a fresh read if that chapter's distillation entry has no `(partial sample)`/`(excerpt)` marker — a marked entry means someone only skimmed it and you should still read it properly if it falls in your sample.

## Key rule: every chapter you read gets distilled

Whenever you read chapter content directly (the fallback path in step 2, or filling a gap), you MUST append your findings to `data/distillations/<slug>.md` in `work-distiller`'s schema (tagged sections: Voice & prose, Structure, Characterization, Emotional mechanics, Canon deviation, Stylistic tics, plus a short beat summary) — not just fold a condensed version into your own per-story profile notes. Mark each chapter section you write `(partial sample)` since you're not doing a full systematic pass, and update the file's "Chapters covered" line to include the chapters you read (it will likely end up non-contiguous, e.g. `1-2, 12, 23 of 23 (partial — representative sample)`). This is what lets `work-distiller` later do a full pass without re-reading what you already covered, and what lets a `preliminary-review` deep dive start from your notes instead of nothing. Do this as you go, per chapter, not batched at the end — same "document as you go" discipline as everywhere else in this pipeline.
...
2. For each story, first check for `data/distillations/<slug>.md` and a `data/preliminary-reviews/<slug>.md` per the source-ranking note above — use whichever exists to avoid a fresh raw-text read. Only when neither exists, or for chapters they don't fully cover, read a representative sample of the raw text: the first 1-2 chapters (establishes voice and hooks), one chapter from the middle third (establishes sustained craft, pacing under length), and the final chapter (establishes how they land endings — a huge crud/not-crud signal, since most fic craters at the finish). For short stories (under ~8 chapters) read the whole thing. Every chapter you read this way gets appended to the distillation per the key rule above — don't let a chapter read pass through your context without leaving a record on disk.
